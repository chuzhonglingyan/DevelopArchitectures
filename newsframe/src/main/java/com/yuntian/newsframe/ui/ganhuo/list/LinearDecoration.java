...
/**
 * description  .
 * Created by ChuYingYan on 2018/5/11.
 */
...
//    }
    ...
    public LinearDecoration(Context context, int orientation, int dividerHeight) {
        ...
        mDividerColor = ContextCompat.getColor(context, R.color.ddd_color);
     ...
     * @param dividerHeight 分割线高度(dp)
     */
    public LinearDecoration(Context context, int orientation, int dividerHeight,int color) {
        if (orientation != LinearLayoutManager.VERTICAL && orientation != LinearLayoutManager.HORIZONTAL) {
            throw new IllegalArgumentException("请输入正确的参数！");
        }
        mOrientation = orientation;
